...
const HELP_SECTIONS := [
	...
	["画面とショートカット", [
		...
		["音", "[key]M[/key] で音のオン・オフ。BGMは朝・昼・夕方・夜で曲が変わり、★が上がるほど豪華になる"],
	...
	["テナントと人", [
		...
		["住宅", "入居すると販売収入（1回だけ。建設費より少し安い）と、毎日の管理費が入る。騒音にとても敏感で、評価が悪い日が続くと退去して返金になる"],
		["ストレス", "エレベーターを待つとたまり、顔が ピンク → 赤 → 赤く点滅（激怒）と変わる。テナントの評価が悪い日が3日続くと退去する"],
		["気持ち", "頭の上のアイコン: 怒りのマーク（激怒）・汗（待たされてストレスが高い）・音符（気分よく着いた）"],
		["騒音", "飲食店・映画館・ロビーなどのまわりに広がり、住宅と客室の評価を下げる"],
	]],
	["お金と評価", [
		["収入と支出", "賃料・宿泊料・売上・入場料 − 維持費 − ゴミの外部委託費。★が1つ上がるごとに賃料と宿泊料が25%増える"],
		["★の条件", "★2: 人口50・警備室 / ★3: 人口120・メディカルセンター・ゴミ処理場 / ★4: 人口250・地下鉄駅・VIPの宿泊 / ★5: 人口500・展望台・結婚式場。★4は不満なテナント（評価が悪い・退去しそう。入居して3日たっていないテナントは数えない）が1割以下、★5は2割以下であることも必要"],
		["昇格と降格", "次の★の条件を全部満たした日が3日続くと★が上がる。今の★の条件を満たさない日が5日続くと★が1つ下がる（不満なテナントは、★を保つだけなら2割以下でよい。上部バーの⚠で知らせる）"],
		["人口", "通勤できる社員 ＋ 客室の定員 ＋ 住宅の入居者"],
		["ゴミと衛生", "ゴミ処理場が足りないとビルが汚れ、全テナントの評価が下がる。続くとゴキブリが出る"],
	]],
	["事件とイベント", [
		["火災", "★2以上でときどき出火し、隣のテナントへ燃え広がることがある（上下の階へは燃え移りにくい）。警備員が手分けして消すので、大きなビルは警備室を5階ごとに1つくらい。ヘリポートの消防ヘリも消す"],
...
func warnings() -> Array[String]:
	...
	if world.rating_system.failing_days > 0:
		list.append("★が下がりそう あと%d日" % (world.rating_system.DEMOTE_DAYS - world.rating_system.failing_days))
	return list
