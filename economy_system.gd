...
var last_report := {} # 最後の決算: {"day", "rent", "hotel", "food", "bonus", "maintenance", "garbage", "garbage_cost", "total"}
...
func settle(day: int) -> void:
	...
	var total := rent + hotel + food + bonus - maintenance - garbage_cost
	last_report = {"day": day, "rent": rent, "hotel": hotel, "food": food, "bonus": bonus, "maintenance": maintenance,
		"garbage": garbage, "garbage_cost": garbage_cost, "total": total}
	world.funds += total
	world.update_funds_display() # last_reportを更新してから表示する（前日の収支も表示されるため）
	# 0円の項目は省いて短くする
	var items: Array[String] = []
	for item in [["賃料", rent], ["宿泊料", hotel], ["飲食", food], ["評価ボーナス", bonus]]:
		if item[1] > 0:
