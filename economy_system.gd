...
var last_report := {} # 最後の決算: {"day", "rent", "hotel", "maintenance", "total"}
...
func settle(day: int) -> void:
	...
	var total := rent + hotel - maintenance
	last_report = {"day": day, "rent": rent, "hotel": hotel, "maintenance": maintenance, "total": total}
	world.funds += total
	world.update_funds_display() # last_reportを更新してから表示する（前日の収支も表示されるため）
	world.show_message("%d日目の決算: 賃料 +%s円 / 宿泊料 +%s円 / 維持費 -%s円 / 合計 %s円" % [
		day, world.format_money(rent), world.format_money(hotel), world.format_money(maintenance),
