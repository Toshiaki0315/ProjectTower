...
いくつかの組に分けて並べて実行すると速く終わります（既定は4組で3分ほど。1組だけなら5分ほど）。

```bash
tools/run_tests.sh
```

分ける数は引数で変えられます（例: `tools/run_tests.sh 3`）。分けすぎると1つあたりの描画が遅くなり、早送りの倍率が下がってかえって遅くなります。
...
- **上部バー（1段目）:** 資金（前日の収支）、日付・曜日・時刻、天気、速度ボタン（押すたびに 1x → 4x → 16x）
- **上部バー（2段目）:** ビルの評価（★）と人口、社員・客室の状況、火事や爆破予告などの知らせ
- **上部バー（3段目）:** 建設メニュー。建てるものを選ぶと、横に建設費と大きさが出ます
...
- **はじめから**: 更地・資金200万円でゲームを始めます。
- **続きから**: ⌘S で保存したセーブデータを読み込んで続きを遊びます（セーブデータがないときは押せません）。
- タイトル画面の間は時間が止まっていて、マップも操作できません。

「はじめから」を押すと、資金200万円で、1日目（4月1日・月曜日）の7:30に、建物が何もない**更地**から始まります。地面の線が1階の床で、その下は地下です。
...
| 撤去する | 建物のどこかを右クリック（建物ごと撤去。建設費の半額が戻る）。建設メニューの「撤去」を選べば左クリックでも撤去できる |
...
| 時間の速さ | 上部バーの速度ボタン（押すたびに 1x → 4x → 16x → 1x と切り替わり、今の速度が出る） |
| 操作説明の開閉 | F1 または H |
...
画面の見方は次のとおりです。
- **上部バー:** 1段目が資金・日付と時刻・速度、2段目がビルの状況（★・人口・社員・客室）、3段目が建設メニュー
...
- **建てたとき:** 白い枠がふわっと広がります。
- **撤去したとき:** 土ぼこりが上がります。
- **決算で黒字のとき:** 入口のあたりに「+424,000円」の文字が浮かび上がります。
...
- 0円の線より上（緑）が黒字、下（赤）が赤字です。棒の高さは、その期間の一番大きい額に合わせて伸び縮みします。
...
**空きフロア（撤去の跡地）:** 上の階を支えているマスを撤去すると、建物の代わりに骨組みだけの「空きフロア」が残ります。上の階はそのままなので、低い階のテナントだけを入れ替えられます。空きフロアは通り抜けでき、その上から別の建物をそのまま建て直せます。上に何もなくなれば、撤去で更地に戻ります。
...
**建物の支え:** 建物は宙に浮かせられません。2階から上の建物は、建てるマスの真下が全部ほかの建物で埋まっている必要があります（1階から上へ積み上げていきます）。地下は、真上に建物がある場所にだけ建てられます（1階から下へ掘り進めます）。ほかの建物を支えている建物を撤去すると、支えを保つために「空きフロア」が残ります（下記）。更地に戻したいときは、上の階から（地下は下の階から）順に撤去します。エレベーターのシャフトも同じく、1階から積み上げ、上のマスから撤去します。

#### テナント（収入になる建物）

| 絵 | 名前 | 大きさ | 建設費 | 内容 |
|---|---|---|---|---|
| ![小さいオフィス](docs/images/buildings/small_office.png) | 小さいオフィス | 横2マス | 24万円 | 社員が2人。賃料は1マス1日**1.1万円**。狭いすき間に入るが、1マスあたりは割高 |
| ![オフィス](docs/images/buildings/office.png) | オフィス | 横4マス | 40万円 | 社員が4人（1マスに1人）。平日8〜9時に出勤し、17〜18時に退社。昼は飲食店かファストフードへ。賃料は1マス1日1万円（休日も入る。空室の間は入らない） |
| ![大きいオフィス](docs/images/buildings/large_office.png) | 大きいオフィス | 横6マス | 48万円 | 社員が6人。賃料は1マス1日**0.9万円**。まとめ借りで1マスあたりは割安だが、6人ぶんの人がエレベーターに乗る |
| ![シングル](docs/images/buildings/hotel.png) | シングル | 横2マス | 15万円 | 17〜21時に客1人が泊まり、翌朝チェックアウトで宿泊料2万円。清掃20分 |
| ![ツイン](docs/images/buildings/hotel_twin.png) | ツイン | 横3マス | 20万円 | 客2人。宿泊料3.5万円。清掃30分 |
| ![スイート](docs/images/buildings/hotel_suite.png) | スイート | 横4マス | 50万円 | 客2人。宿泊料8万円。清掃45分 |
| ![飲食店](docs/images/buildings/restaurant.png) | 飲食店 | 横3マス | 20万円 | 平日12〜13時に社員が一番近い店で昼食（30分）。売上は1人1,000円。外からのお客さんも来る（平日2人・休日8人） |
| ![ファストフード](docs/images/buildings/fastfood.png) | ファストフード | 横2マス | 12万円 | 飲食店の小さくて速い版。食事は10分、売上は1人600円。狭い場所に置けて、昼の混雑をさばける。外からのお客さんも来る（平日3人・休日10人） |
| ![ショップ](docs/images/buildings/shop.png) | ショップ | 横3マス | 25万円 | 外からのお客さんが買い物に来る（平日3人・休日10人、1人1,500円、滞在20分）。社員は来ない（維持費 3,000円/日） |
| ![住宅](docs/images/buildings/housing.png) | 住宅 | 横3マス | 40万円 | 3人家族が入居すると販売収入70万円（1回だけ）。家族は毎日出かけて帰ってくる |
| ![結婚式場](docs/images/buildings/wedding.png) | 結婚式場 | 横6マス | 100万円 | 休日の10〜11時に来客12人、13時まで。1人1万円 |
| ![イベントホール](docs/images/buildings/event_hall.png) | イベントホール | 横6マス | 80万円 | 休日の13〜14時に来客15人、17時まで。1人3,000円 |
| ![映画館](docs/images/buildings/cinema.png) | 映画館 | 横8マス・高さ2階分 | 150万円 | 上映時刻に客が一斉に来て、2時間後に一斉に帰る（1人1,800円）。平日は13時・16時・19時、休日は11時・14時・17時・20時。1回の客は平日10人・休日16人で、使える駐車場の台数ぶん増える（最大8人）。人が歩くのは下の階だけ（維持費 2万円/日） |
...
| 絵 | 名前 | 大きさ | 建設費 | 内容 |
|---|---|---|---|---|
| ![ロビー](docs/images/buildings/lobby.png) | ロビー | 1マス | 1.5万円/マス | 1階専用。1マスずつ横に伸ばせる。左端が入口になる（一晩中明かりが灯る） |
| ![吹き抜けロビー（2階分）](docs/images/buildings/lobby2.png) | 吹き抜けロビー（2階分） | 1マス・高さ2階分 | 3万円/マス | 天井の高いロビー。上の階には床がないので、人が歩くのは1階だけ |
| ![吹き抜けロビー（3階分）](docs/images/buildings/lobby3.png) | 吹き抜けロビー（3階分） | 1マス・高さ3階分 | 4.5万円/マス | 同じく3階分の高さ |
| ![スカイロビー](docs/images/buildings/sky_lobby.png) | スカイロビー | 1マス | 5万円/マス | 15階・30階・45階…にだけ建てられる乗り換え専用のフロア。1マスずつ横に伸ばせる（入口にはならない） |
| ![階段](docs/images/buildings/stairs.png) | 階段 | 1マス | 5万円 | そのマスと1つ上の階をつなぐ。縦に積めば何階でも行ける。近い階の移動向き |
| ![エスカレーター](docs/images/buildings/escalator.png) | エスカレーター | 横2マス | 10万円 | 左のマス（乗り口）から、1つ上の階の右のマスへ斜めに上がる（下りにも使える）。待ち時間も定員もなく、階段より楽で速い。降り口に次のエスカレーターを建てると乗り継げる（維持費 2,000円/日） |
| ![エレベーター](docs/images/buildings/elevator.png) | エレベーター | 1マス | 8万円/マス | 縦に並べたマスが1本のシャフトになり、カゴが1台できる。遠い階の移動向き（維持費 2,000円/マス/日） |
| ![大型エレベーター](docs/images/buildings/large_elevator.png) | 大型エレベーター | 横2マス | 24万円/階 | **全部の階に停まる**大きなエレベーター。カゴは標準の1.5倍の速さで定員16人（維持費 3,000円/階/日）。中くらいの階の混雑をさばくのに向く |
| ![急行エレベーター](docs/images/buildings/express_elevator.png) | 急行エレベーター | 1マス | 12万円/マス | 1階とスカイロビーの階（15階・30階…）にだけ停まる。途中の階では乗り降りできない。カゴは標準の3倍の速さで定員20人（維持費 3,000円/マス/日） |
| ![サービスエレベーター](docs/images/buildings/service_elevator.png) | サービスエレベーター | 1マス | 8万円/マス | 裏方専用のエレベーター。清掃員だけが乗れて、社員や客は乗れない。客室と裏方の動線を分けると、普通のエレベーターが空く（維持費 1,500円/マス/日） |
| ![カゴ](docs/images/elevator/closed.png) | カゴ追加 | － | 5万円/台 | シャフトにカゴを増やす（1本に4台まで。維持費 3,000円/台/日） |
...
|---|---|---|---|---|---|
| ![ハウスキーパー室](docs/images/buildings/housekeeping.png) | ハウスキーパー室 | 横2マス | 20万円 | 1万円 | 清掃員が2人。清掃待ちの客室を近い順に掃除する |
| ![ゴミ処理場](docs/images/buildings/recycling.png) | ゴミ処理場 | 横3マス | 15万円 | 5,000円 | 1施設で1日20のゴミを処理。足りない分は外部委託（1につき1,000円）。足りない日が続くとビルが汚れる |
| ![警備室](docs/images/buildings/security.png) | 警備室 | 横2マス | 10万円 | 5,000円 | 警備員が1人常駐し、爆弾の解体と消火に向かう。★2にも必要 |
| ![メディカルセンター](docs/images/buildings/medical.png) | メディカルセンター | 横3マス | 20万円 | 1万円 | ビル全体のストレスの回復が速くなる（1施設で1.5倍、増やすほど速く、最大2.5倍）。★3にも必要 |
| ![地下鉄駅](docs/images/buildings/subway.png) | 地下鉄駅 | 横4マス | 100万円 | 1万円 | **地下5階より深いところ**にだけ建てられる。2つ目の入口になり、店・映画館のお客さんが1駅につき5割増える（最大2倍）。★4にも必要 |
| ![スロープ](docs/images/buildings/ramp.png) | スロープ | 横2マス | 20万円 | 2,000円 | 車が下りてくる道。地下1階のスロープが1階からの入口で、その真下の階にもスロープを建てると、さらに下の階まで車で下りられる |
| ![地下駐車場](docs/images/buildings/parking.png) | 地下駐車場 | 横4マス | 30万円 | 5,000円 | 1棟に4台。地下の何階にでも建てられるが、**同じ階**のスロープまで行ける駐車場だけが使える（車は階段では下りられない） |
| ![ヘリポート](docs/images/buildings/helipad.png) | ヘリポート | 横4マス | 80万円 | 1万円 | 屋上（上に建物がないところ）にだけ建てられる。火事のとき、消防ヘリが飛んできて上の階の火から消す |
| ![屋上庭園](docs/images/buildings/garden.png) | 屋上庭園 | 横4マス | 40万円 | 5,000円 | 屋上にだけ建てられる。ビル全体のストレスの回復が1.3倍になり（最大2.5倍まで重なる）、騒音を3ぶんやわらげる |
...
- 金額は深さ1階につき1〜2万円（地下20階なら20〜40万円ほど、上限100万円）。
- 「埋蔵金を発見！ B12階で 240,000円 を掘り当てました」とメッセージが出て、資金に入ります。
...
- 1マスが**60分**燃え続けると、そのテナントは**焼け落ちます**（払い戻しなし）。
...
- 120分たつと爆発して、**そのテナントが吹き飛びます**（払い戻しはありません）。
...
毎日0時に前日の分を計算し、資金に反映します。結果は下部バーのメッセージと、資金の横の「（前日 +○○円）」でわかります。

| 収入 | 支出 |
|---|---|
| オフィスの賃料（1マス1日 小1.1万円・普通1万円・大0.9万円。休日も入る） | 維持費（エレベーター・カゴ・設備・店） |
| ホテルの宿泊料（チェックアウトした日） | ゴミの外部委託費（ゴミ処理場で処理しきれない分、1につき1,000円） |
| 飲食店の売上 | |
| 住宅の販売収入（入居したとき1回だけ） | |
| 結婚式場・イベントホールの料金 | |
