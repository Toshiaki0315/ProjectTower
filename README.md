...
- **上部バー（1段目）:** 資金（前日の収支）、日付・曜日・時刻、速度ボタン（1x / 4x / 16x）、「操作説明」ボタン
- **上部バー（2段目）:** 建設メニュー。建てるものを選ぶと、横に建設費と大きさが出ます
- **マップ:** ビルの断面図です。空の色は時刻とともに変わり、昼は太陽、夜は月と星が出ます（[昼と夜](#昼と夜)）。1階の床より下は地下（土）です
- **右端:** 上下のスクロールバー
- **下部バー（1段目）:** 操作の結果や決算などのメッセージ
- **下部バー（2段目）:** ビルの評価と人口、社員・客室の状況、カーソル下のマスの情報

最初は資金200万円で、1日目（月曜日）の7:30に、建物が何もない**更地**から始まります。地面の線が1階の床で、その下は地下です。
