# ProjectTower

名作『SimTower』をリスペクトした、2Dサイドビューのビル経営シミュレーションです。
テナントを建ててビルを大きくしながら、エレベーターを工夫して住人の待ち時間（ストレス）を減らし、ビルの評価（★）を上げていきます。

![昼のビル（昼休みで飲食店とエレベーターが混み合っている）](docs/images/screenshots/day.png)
![夜のビル（人がいる部屋と設備にだけ明かりが灯る）](docs/images/screenshots/night.png)

## 目次

- [開発環境の構築](#開発環境の構築)
- [アプリの実行方法](#アプリの実行方法)
- [遊び方（マニュアル）](#遊び方マニュアル)
  - [画面の見方](#画面の見方)
  - [操作方法](#操作方法)
  - [1日の流れ](#1日の流れ)
  - [建てられるもの](#建てられるもの)
  - [人の種類](#人の種類)
  - [人の気持ち（ストレス）](#人の気持ちストレス)
  - [エレベーター](#エレベーター)
  - [お金（毎日の決算）](#お金毎日の決算)
  - [ビルの評価（★）](#ビルの評価)
- [ファイル構成](#ファイル構成)
- [開発の進め方](#開発の進め方)

---

## 開発環境の構築

### 必要なもの

| もの | バージョン | 用途 |
|---|---|---|
| [Godot Engine](https://godotengine.org/) | 4.7（4.7.2 で動作確認） | ゲームエンジン（エディタと実行） |
| Git | 何でもよい | ソースコードの取得 |

画像ファイルや追加のアセットは不要です。建物・人のドット絵やUIは、すべてGDScriptのコードで作っています。

### 手順（macOS）

1. **Godot をインストールする**

   Homebrew を使う場合:

   ```bash
   brew install --cask godot
   ```

   または [Godot の公式サイト](https://godotengine.org/download) から「Godot Engine 4.7（標準版）」をダウンロードして、アプリケーションフォルダに入れます。
   コマンドラインから `godot` を使えるか確かめます:

   ```bash
   godot --version
   ```

2. **ソースコードを取得する**

   ```bash
   git clone git@github.com:Toshiaki0315/ProjectTower.git
   ```

3. **Godot エディタでプロジェクトを開く**

   Godot を起動し、プロジェクトマネージャーの「インポート」から、取得したフォルダの `project.godot` を選びます。
   初回はエディタが画像などの読み込み（インポート）を行うので、少し待ちます。

## アプリの実行方法

### Godot エディタから

プロジェクトを開いて **F5**（または右上の ▶ ボタン）を押すと、ゲームが始まります。

### コマンドラインから

```bash
godot --path .
```

（`ProjectTower` フォルダの中で実行します）

### 自動テスト（画面確認テスト）

実際のクリック操作を再現して、建設・移動・エレベーター・収支などを確かめ、各場面のスクリーンショットを保存します。
ウィンドウが開いて約4〜5分かかります（テスト中のウィンドウはマウスを受け付けないので、そのまま待ちます）。

```bash
godot --path . -s res://tests/screenshot_test.gd -- /tmp/projecttower_screens
```

最後に `RESULT: ALL PASSED` と出れば成功です。`--` の後ろはスクリーンショットの保存先です（省略すると Godot のユーザーデータフォルダに保存）。

### README の画像を作り直す

ドット絵を変えたときは、次のコマンドで `docs/images/` の画像を作り直せます。

```bash
godot --path . -s res://tools/generate_readme_images.gd
```

---

## 遊び方（マニュアル）

### 画面の見方

- **上部バー（1段目）:** 資金（前日の収支）、日付・曜日・時刻、速度ボタン（1x / 4x / 16x）、「操作説明」ボタン
- **上部バー（2段目）:** 建設メニュー。建てるものを選ぶと、横に建設費と大きさが出ます
- **マップ:** ビルの断面図です。空の色は時刻とともに変わり、昼は太陽、夜は月と星が出ます。1階の床より下は地下（土）です
- **右端:** 上下のスクロールバー
- **下部バー（1段目）:** 操作の結果や決算などのメッセージ
- **下部バー（2段目）:** ビルの評価と人口、社員・客室の状況、カーソル下のマスの情報

最初は資金100万円で、1日目（月曜日）の7:30から始まります。最初からあるオフィスのブロック（4階建て）の一番下の段が1階です。

### 操作方法

| 操作 | やり方 |
|---|---|
| 建てる | 上の「建設」メニューで選び、マップを左クリック（クリックしたマスが建物の左端） |
| 撤去する | 建物のどこかを右クリック（建物ごと撤去。建設費の半額が戻る） |
| 上下スクロール | マウスホイール / 右端のスクロールバー / W・S・↑・↓ キー |
| 左右スクロール | Shift + マウスホイール / A・D・←・→ キー |
| ズーム | Ctrl（⌘）+ マウスホイール / トラックパッドのピンチ |
| カメラを動かす | トラックパッドの2本指スクロール / マウスの中ボタンでドラッグ |
| 時間の速さ | 上部バーの 1x / 4x / 16x |
| エレベーターを呼ぶ | 「エレベーター」を選んだ状態で、シャフトのマスをクリック |
| カゴを増やす | 「カゴ追加」を選んで、シャフトのマスをクリック |
| 住人を動かして試す | 「住人（テスト）」を選び、建物をクリックで住人を置き、行き先をクリック |

カーソルを置くと、建てるマスが緑（建てられる）か赤（建てられない）で表示されます。

### 1日の流れ

ゲーム内の1時間は、1x で現実の30秒です。1日目は月曜日で、土日は休日です。

| 時刻 | 平日 | 休日（土日） |
|---|---|---|
| 7:00〜9:00 | 住宅の家族が出かける | |
| 7:00〜10:00 | ホテルの客がチェックアウト（部屋は清掃待ちに） | 同じ |
| 8:00〜9:00 | 社員が出勤（朝のラッシュ） | オフィスは休み |
| 10:00〜12:00 | | 住宅の家族が出かける |
| 10:00〜13:00 | | 結婚式（来客12人） |
| 12:00〜13:30 | 社員が飲食店で昼食（昼のラッシュ） | |
| 13:00〜17:00 | | イベント（来客15人） |
| 15:00〜18:00 | | 住宅の家族が帰ってくる |
| 17:00〜18:00 | 社員が退社（夕方のラッシュ） | |
| 17:00〜20:00 | 住宅の家族が帰ってくる | |
| 17:00〜21:00 | ホテルに客がチェックイン | 同じ |
| 0:00 | 前日の決算（収支が資金に反映） | 同じ |

人はビルの**入口**から出入りします。入口は1階の左端（緑の扉の印）と、地下鉄駅（水色の扉の印）で、近い方が使われます。入口からたどり着けない建物には人が来ません。

### 建てられるもの

建物はそれぞれ横に何マスかの大きさがあり、クリックしたマスを左端に建ちます。上下の階は、**階段**か**エレベーター**でつながないと行き来できません。

#### テナント（収入になる建物）

| 絵 | 名前 | 大きさ | 建設費 | 内容 |
|---|---|---|---|---|
| ![オフィス](docs/images/buildings/office.png) | オフィス | 横4マス | 40万円 | 社員が4人（1マスに1人）。平日8〜9時に出勤し、17〜18時に退社。昼は飲食店へ。賃料は1マス1日1万円（休日も入る） |
| ![シングル](docs/images/buildings/hotel.png) | シングル | 横2マス | 15万円 | 17〜21時に客1人が泊まり、翌朝チェックアウトで宿泊料2万円。清掃20分 |
| ![ツイン](docs/images/buildings/hotel_twin.png) | ツイン | 横3マス | 20万円 | 客2人。宿泊料3.5万円。清掃30分 |
| ![スイート](docs/images/buildings/hotel_suite.png) | スイート | 横4マス | 50万円 | 客2人。宿泊料8万円。清掃45分 |
| ![飲食店](docs/images/buildings/restaurant.png) | 飲食店 | 横3マス | 20万円 | 平日12〜13時に社員が一番近い店で昼食（30分）。売上は1人1,000円 |
| ![住宅](docs/images/buildings/housing.png) | 住宅 | 横3マス | 40万円 | 3人家族が入居すると販売収入70万円（1回だけ）。家族は毎日出かけて帰ってくる |
| ![結婚式場](docs/images/buildings/wedding.png) | 結婚式場 | 横6マス | 100万円 | 休日の10〜11時に来客12人、13時まで。1人1万円 |
| ![イベントホール](docs/images/buildings/event_hall.png) | イベントホール | 横6マス | 80万円 | 休日の13〜14時に来客15人、17時まで。1人3,000円 |

ホテルの客室は、チェックアウトすると「清掃待ち」（茶色の汚れ）になり、清掃が済むまで次の客は泊まれません。**ハウスキーパー室**を建てておきましょう。

#### 移動（上下の階をつなぐ）

| 絵 | 名前 | 大きさ | 建設費 | 内容 |
|---|---|---|---|---|
| ![階段](docs/images/buildings/stairs.png) | 階段 | 1マス | 5万円 | そのマスと1つ上の階をつなぐ。縦に積めば何階でも行ける。近い階の移動向き |
| ![エレベーター](docs/images/buildings/elevator.png) | エレベーター | 1マス | 10万円/マス | 縦に並べたマスが1本のシャフトになり、カゴが1台できる。遠い階の移動向き（維持費 2,000円/マス/日） |
| ![カゴ](docs/images/elevator/closed.png) | カゴ追加 | － | 5万円/台 | シャフトにカゴを増やす（1本に4台まで。維持費 3,000円/台/日） |

人は移動の手間（歩く距離・階段・エレベーターの待ち時間）を比べて、一番楽な経路を選びます。1〜2階の移動なら階段、それより遠い階ならエレベーターを使うことが多くなります。

#### 設備

| 絵 | 名前 | 大きさ | 建設費 | 維持費/日 | 内容 |
|---|---|---|---|---|---|
| ![ハウスキーパー室](docs/images/buildings/housekeeping.png) | ハウスキーパー室 | 横2マス | 20万円 | 1万円 | 清掃員が2人。清掃待ちの客室を近い順に掃除する |
| ![ゴミ処理場](docs/images/buildings/recycling.png) | ゴミ処理場 | 横3マス | 15万円 | 5,000円 | 1施設で1日20のゴミを処理。足りない分は外部委託（1につき1,000円） |
| ![警備室](docs/images/buildings/security.png) | 警備室 | 横2マス | 10万円 | 5,000円 | ★2に必要 |
| ![メディカルセンター](docs/images/buildings/medical.png) | メディカルセンター | 横3マス | 20万円 | 1万円 | ★3に必要 |
| ![地下鉄駅](docs/images/buildings/subway.png) | 地下鉄駅 | 横4マス | 100万円 | 1万円 | 地下にだけ建てられる。2つ目の入口になる。★4に必要 |

### 人の種類

服の色で種類がわかります。カーソルを人のいるマスに合わせると、下部バーにその人のストレスが出ます。

| 絵 | 種類 | どこから来て、何をするか |
|---|---|---|
| ![社員](docs/images/people/worker.png) | 社員（白） | オフィスに通勤する。昼は飲食店へ |
| ![宿泊客](docs/images/people/guest.png) | 宿泊客（薄紫） | 夕方にホテルに来て、翌朝帰る |
| ![清掃員](docs/images/people/housekeeper.png) | 清掃員（水色） | ハウスキーパー室にいて、客室を掃除しに行く |
| ![入居者](docs/images/people/housing.png) | 住宅の入居者（緑） | 住宅に住み、毎日出かけて帰ってくる |
| ![結婚式の来客](docs/images/people/wedding.png) | 結婚式の来客（クリーム色） | 休日に結婚式場に来る |
| ![イベントの来客](docs/images/people/event.png) | イベントの来客（オレンジ） | 休日にイベントホールに来る |
| ![選択中](docs/images/people/selected.png) | 選択中（黄色） | 「住人（テスト）」で置いて、行き先を待っている人 |

### 人の気持ち（ストレス）

人はエレベーターを待っている間にストレスがたまります（0〜100）。ストレスが高くなると服の色が変わるので、どこで待たされているかが一目でわかります。

| 絵 | ストレス | 気持ち |
|---|---|---|
| ![平常](docs/images/people/stress_low.png) | 0〜39 | 平常（服はその人の種類の色） |
| ![イライラ](docs/images/people/stress_mid.png) | 40〜69 | イライラ（ピンク） |
| ![とても不満](docs/images/people/stress_high.png) | 70〜100 | とても不満（赤） |

- **たまる:** エレベーターを待っている間、ゲーム内の1分につき約5
- **変わらない:** 歩いている間・カゴに乗っている間
- **回復する:** 目的地に着いて落ち着いている間、ゲーム内の1分につき約2.5

待っている人の頭の上には「…」が出ます。ストレスを減らすには、エレベーターのカゴを増やしたり、シャフトを増やしたり、近い階なら階段を置いたりします。

### エレベーター

![カゴ（扉が閉まっている）](docs/images/elevator/closed.png) ![カゴ（扉が開いている・満員）](docs/images/elevator/open_full.png)

- **定員は8人:** カゴの下のゲージが乗っている人数で、満員になると赤くなります。満員のカゴは乗り場の呼び出しでは停まらず、乗れなかった人は次のカゴを待ちます。
- **進行方向:** カゴの上の緑の ▲▼ が進行方向です。カゴは進んでいる方向の先に呼び出しがある限り進み続け、途中の呼ばれた階に寄ります（集合制御）。待っている人は、行きたい方向へ進むカゴにだけ乗ります。
- **複数のカゴ（群管理）:** 1本のシャフトに4台までカゴを置けます。乗り場の呼び出しは、到着までの手間（距離・進行方向・停まる予定の数・混み具合）が一番小さいカゴに割り当てられます。朝のラッシュでは、1台だと10時過ぎまでかかる出勤が、4台なら9時半ごろに終わります。
- **カーソル:** シャフトにカーソルを合わせると「カゴ4台: 5/8・0/8・2/8・0/8人」のように各カゴの人数が出ます。

### お金（毎日の決算）

毎日0時に前日の分を計算し、資金に反映します。結果は下部バーのメッセージと、資金の横の「（前日 +○○円）」でわかります。

| 収入 | 支出 |
|---|---|
| オフィスの賃料（1マス1日1万円。休日も入る） | 維持費（エレベーター・カゴ・設備） |
| ホテルの宿泊料（チェックアウトした日） | ゴミの外部委託費（ゴミ処理場で処理しきれない分、1につき1,000円） |
| 飲食店の売上 | |
| 住宅の販売収入（入居したとき1回だけ） | |
| 結婚式場・イベントホールの料金 | |
| 評価ボーナス（★に応じて賃料と宿泊料に上乗せ） | |

ゴミは、出勤があったオフィス1マスにつき1、チェックアウトした客室1室につき1、入居済みの住宅1戸につき1、飲食店の客・会場の来客10人につき1出ます。

### ビルの評価（★）

毎日の決算のときに条件を満たしていれば★が1つ上がります（下がることはありません）。★が1つ上がるごとに、賃料と宿泊料に25%の評価ボーナスが付きます（昇格した翌日の決算から）。

| 評価 | 条件 |
|---|---|
| ★1 | 最初の評価 |
| ★2 | 人口50以上、警備室がある |
| ★3 | 人口120以上、メディカルセンターとゴミ処理場がある |
| ★4 | 人口250以上、地下鉄駅がある |

**人口** = 通勤できる社員の数 + 客室の定員 + 住宅の入居者の数。下部バーの左端に、今の評価と次の★に足りないものが出ます。

---

## ファイル構成

UIやゲームの仕組みはすべてコード（GDScript）で作っています（コードファースト）。エディタで作ったのは `main.tscn`（タイルマップとカメラ）だけです。

| ファイル | 役割 |
|---|---|
| `main.tscn` / `main.gd` | ゲーム全体。建物の定義（`BUILDINGS`）、建設・撤去、UI、移動ルールと経路探索（ダイクストラ法） |
| `pixel_art.gd` | 建物のドット絵（文字で描いた絵と色の表） |
| `resident.gd` | 人（歩く・エレベーターを待つ・乗る、ストレス、ドット絵） |
| `elevator_car.gd` | エレベーターのカゴ（集合制御・定員・手間の見積もり） |
| `elevator_system.gd` | シャフトとカゴの管理、カゴの追加、群管理 |
| `game_clock.gd` | 時計・曜日、空の色、太陽と月 |
| `commute_system.gd` | 社員の出勤・退社 |
| `commerce_system.gd` | 飲食店の昼食 |
| `hotel_system.gd` | ホテルの客室・宿泊客・清掃員 |
| `housing_system.gd` | 住宅と入居者の家族 |
| `event_system.gd` | 結婚式場・イベントホールの来客 |
| `economy_system.gd` | 毎日の決算（収入・維持費・ゴミ） |
| `rating_system.gd` | ビルの評価（★） |
| `lighting.gd` | 夜の建物の暗さと部屋の明かり |
| `grid_overlay.gd` | マス目・枠線・カーソル表示、空・星・太陽と月・土の背景 |
| `camera_controller.gd` | カメラ（スクロール・ズーム） |
| `tests/screenshot_test.gd` | 画面確認テスト（クリック操作を再現して確かめ、スクリーンショットを保存） |
| `tools/generate_readme_images.gd` | README の画像を作るスクリプト |

## 開発の進め方

- [Project_Overview.md](Project_Overview.md): ゲームのコンセプトと登場アイテム
- [Tasks.md](Tasks.md): 開発のフェーズとタスク（完了したものにチェック）
- [Tech_Stack.md](Tech_Stack.md): 技術選定
- [Claude.md](Claude.md): AIアシスタントに開発を依頼するときのルール
