...
| 撤去する | 建物のどこかを右クリック（建物ごと撤去）。**撤去費用がかかります**（建設費の1割・最低2,000円。建設費は戻りません）。建設メニューの「撤去」を選べば左クリックでも撤去できる |
...
**焼け跡:** 火事で焼け落ちたり、爆発で吹き飛んだりした部屋は、**黒焦げの焼け跡**になります。上の階は支えたままなので浮きませんが、焼け跡の上にはそのまま建てられません。**撤去（1マス2,000円）**してから建て直します。上に建物が残っていれば、撤去したあとは空きフロアになります。
