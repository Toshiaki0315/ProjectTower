...
- **上部バー（2段目）:**
  - **★** … ビルの評価。押すと**ビルの状況**（人口・次の★の条件・目標・社員・オフィス・客室）が開きます。気をつけることがあるときは、★の横に **⚠と件数**が出ます（怒っている人・退去しそうなテナント・通勤できない社員・衛生の悪化・ゴキブリ・火災・爆破予告）
...
**はじめの一歩**
1. 建設メニューで「ロビー」を選び、1階（地面の線のすぐ上の段）をクリックして横に並べます。ロビーの左端と右端が出入り口になり、人はここから出入りします。
...
ゲームには4つの目標があり、上から順に1つずつ挑戦します。今の目標は、★を押して開く「ビルの状況」に「目標: 45日目までに★3（あと12日）」のように出ます。
...
- **1階の左の出入り口**（ロビーの左端）
- **1階の右の出入り口**（ロビーの右端。ロビーが1マスだけなら左と同じ1つ）
- **地下鉄駅**
...
| 絵 | 名前 | 大きさ | 建設費 | 内容 |
|---|---|---|---|---|
| ![ロビー](docs/images/buildings/lobby.png) | ロビー | 1マス | 1.5万Cr/マス | 1階専用。1マスずつ横に伸ばせる。左端と右端が出入り口になる（一晩中明かりが灯る） |
